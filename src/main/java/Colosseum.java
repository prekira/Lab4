...
public class Colosseum {
    ...
    public static Pokemon buildPokemon() {
        ...
        String nameP = myScan.next();
        if (nameP != null && nameP.length() > 0) {
            tempPokemon.name = nameP;
        }

        System.out.println("How many hit points will it have? (1-50)");
        int hitPointsP = myScan.nextInt();
        while (hitPointsP < 1 || hitPointsP > MAX_HIT_POINTS) {
            System.out.println("Sorry. Hit points must be between 1 and 50:");
            hitPointsP = myScan.nextInt();
        }
        tempPokemon.hitPoints = hitPointsP;

        int sum = -1;
        while (sum <= 50) {
            System.out.println("Split fifty points between attack level and defense level");
            System.out.println("Enter your attack level (1-49):");
            int attackLevelP = myScan.nextInt();
            while (attackLevelP < 1 || attackLevelP > 49) {
                System.out.println("Sorry. The attack level must be between 1 and 49:");
                System.out.println("Enter your attack level (1-49):");
                attackLevelP = myScan.nextInt();
            }
            tempPokemon.attackLevel = attackLevelP;


            System.out.println("* Enter your defense level (1-3):");
            int defenseLevelP = myScan.nextInt();
            while (defenseLevelP < 1 || defenseLevelP > 49) {
                System.out.println("Sorry. The defense level must be between 1 and 49:");
                System.out.println("Enter your defense level (1-49):");
                defenseLevelP = myScan.nextInt();
            }
            tempPokemon.defenseLevel = defenseLevelP;

            sum = defenseLevelP + attackLevelP;
            if (sum <= 50) {
                break;
            } else {
                System.out.println("Split fifty points between attack level and defense level");
